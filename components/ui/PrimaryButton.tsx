...
const PrimaryButton: React.FC<{ text?: string, textStyles?: string, styles?: string, isLoading?: boolean, type?: "button" | "submit" | "reset", icon?: any, onClick?: () => void }> = ({ text, styles, type, textStyles, icon, onClick, isLoading }) => {
    return (
        <button type={type} disabled={isLoading} className={`bg-primary ${isLoading ? `px-2 ${icon ? "" : ""} opacity-80 text-center cursor-not-allowed` : `${icon ? "" : ""} cursor-pointer`} transition duration-300 h-max text-white rounded-full inter ${styles}`} onClick={onClick}>
            {!isLoading ?
                (
