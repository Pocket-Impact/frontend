...
const Navbar = () => {
    ...
    return (
        <>
            <div className={`px-4 md:flex hidden z-50 bg-background/40 backdrop-blur-xs rounded-b-2xl fixed w-full max-w-[1440px] justify-between inter h-16 items-center p-2 left-1/2 top-0 -translate-x-[50%] ${borderClass}`}>
            ...
            <div className={`max-md:flex flex-col ${borderClass} hidden ${open ? "outline outline-primary" : ""} px-4 bg-background z-20 rounded-b-2xl pb-5 fixed w-full max-w-[1440px] justify-between inter transition-all duration-300 overflow-hidden ${!open ? "h-16" : "h-56 outline"} items-center p-2 left-1/2 top-0 -translate-x-[50%] z-10`}>
