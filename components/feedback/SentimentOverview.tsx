    ...
    series: [
      {
        ...
        data: sentimentAnalysis.map((entry) => ({
          value: entry.value,
          name: entry.name,
          itemStyle: { color: sentimentColorMap[entry.name] || "#3b82f6" },
        })),
      },
    ],
  };
  ...
  return (
    ...
    >
      <div className="flex flex-col">
        <h2 className="lg font-semibold">Sentimental distribution</h2>
      ...
      <div className="w-full h-full flex flex-col items-center">
        {sentimentAnalysis.length > 0 ? (
              ...
              <div className="flex flex-col items-start px-2 w-max justify-center my-4 gap-2">
                <div className="flex items-center gap-2 font-medium relative min-h-6">
                  ...
                  <span className="sm">
                    Positive -{" "}
                    {sentimentAnalysis
                      .find((entry) => entry.name === "Positive")
                      ?.value?.toString()
                      .padStart(2, "0")}{" "}
                    feedbacks
                  </span>
                ...
                <div className="flex items-center gap-2 font-medium relative min-h-6">
                  ...
                  <span className="sm">
                    Negative -{" "}
                    {sentimentAnalysis
                      .find((entry) => entry.name === "Negative")
                      ?.value?.toString()
                      .padStart(2, "0")}{" "}
                    feedbacks
                  </span>
          ...
          <div className="w-full h-full mt-4 py-3 bg-black/5 text-black/80 rounded-sm flex items-center justify-center">
            No sentimental data available
          </div>
        )}
      </div>
    </div>
